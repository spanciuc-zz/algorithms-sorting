...
class SortingTest {
  ...
  protected BinaryInsertion sorting;

  @BeforeEach
  void setUp() {
    sorting = new BinaryInsertion();
  }
  ...
  void sortAscending_whenParamNotNullAndNotEmpty_shouldSortAscendingAndReturn() {
    var expected = createExpectedAscending();
    var unordered = createUnordered();
    System.out.println(Arrays.stream(unordered)
        .toList());
    System.out.println(Arrays.stream(sorting.sortAscending(unordered))
        .toList());
